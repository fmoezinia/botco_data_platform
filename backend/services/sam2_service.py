    ...
    async def process_video(
        ...
        try:
            ...
            result = _sam2_video_processing_task(
                task_id=task_id or f"sam2_task_{int(time.time())}", 
            ...
            visualization_path = visualization_data.get('visualization_path') if visualization_data else None
            
            if visualization_path:
                logger.info(f"Visualization generated: {visualization_path}")
            else:
                logger.warning("No visualization path found in result")
